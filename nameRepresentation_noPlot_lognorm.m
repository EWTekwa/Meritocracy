function [authorLikeRatio,topAcadLikeRatio,botAcadLikeRatio,topCapLikeRatio,botCapLikeRatio,Port_top,rankMeritSlope,capMeritSlope,capMeritCor,LRMeritSlope,Merit_pop,Merit_top,Merit_acad,Capital_pop,Capital_top,Capital_acad]=nameRepresentation_noPlot_lognorm(numGen,numNames,initPopPerName,nameDistr,initAcademiaPop,reprodRate,reprodCost1,reprodCost2,evaluation,merit_mean,merit_var,capital_mean,capital_var,MeritCapCausal,mateChoice,merit_herit,capital_herit,capital_added,capital_added_var,child_surname,child_surnameRelVar,nameMutation,distr)
...
for name=1:numNames %only assign names to initiated individuals among initially existing surnames
    if distr==1 %normal distributions
    ...
    else %log-normal distributions
        mu=log(merit_mean/sqrt(merit_var/(merit_mean)^2+1));
        sigma=sqrt(log(merit_var/(merit_mean)^2+1));
        GenIndiv_merit(name,1:GenIndiv(name),1)=lognrnd(mu,sigma,1,GenIndiv(name));
        capital_temp=(1-MeritCapCausal)*capital_mean+MeritCapCausal*(GenIndiv_merit(name,1:GenIndiv(name),1));
        mu=log(capital_temp/sqrt(capital_var./(capital_temp).^2+1));
        sigma=sqrt(log(capital_var./(capital_temp).^2+1));
        GenIndiv_capital(name,1:GenIndiv(name),1)=lognrnd(mu,sigma); %additional causal relationship between capital and merit
    end
end
                ...
                if children>0
                    ...
                    for child=1:children
                        ...
                        if distr==1
                            ...
                            merit_temp=merit_herit*mean(GenIndiv_merit_t(ParentOrder(2*i-1:2*i)))+(1-merit_herit)*(merit_mean);
                            mu=log(merit_temp/sqrt(merit_var./(merit_temp).^2+1));
                            sigma=sqrt(log(merit_var./(merit_temp).^2+1));
                            GenIndiv_merit(surname_child,surname_cur+1,t+1)=lognrnd(mu,sigma);
                            capital_temp=capital_herit*mean(GenIndiv_capital_t(ParentOrder(2*i-1:2*i)))+(1-capital_herit)*((1-MeritCapCausal)*capital_mean+MeritCapCausal*GenIndiv_merit(surname_child,surname_cur+1,t+1));
                            mu=log(capital_temp/sqrt(capital_var./(capital_temp).^2+1));
                            sigma=sqrt(log(capital_var./(capital_temp).^2+1));
                            GenIndiv_capital(surname_child,surname_cur+1,t+1)=lognrnd(mu,sigma); %additional causal relationship between capital and merit
                        end
                    end
                end
            end
